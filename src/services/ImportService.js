...
const { User, Team, Organization } = require('../models');
const { auditLogger } = require('../utils/auditLogger');

class ImportService {
  static async importUsersFromFile(filePath, organizationId, importedBy, options = {}) {
    try {
      const results = {
        total: 0,
        successful: 0,
      ...
      if (organization.maxUsers) {
        const currentUserCount = await User.count({ where: { organizationId } });
        const availableSlots = organization.maxUsers - currentUserCount;
        
        if (userData.length > availableSlots) {
          throw new Error(`Превышен лимит пользователей. Доступно: ${availableSlots}, запрошено: ${userData.length}`);
        }
      }
      ...
      for (const [index, userRow] of userData.entries()) {
        try {
          const processedUser = await this.processUserRow(userRow, organizationId, options);
          if (processedUser) {
  ...
  static async processUserRow(userRow, organizationId, options = {}) {
    ...
    const fieldMapping = {
      ...
      telegram_username: ['telegram_username', 'tg_username', 'telegram user', 'tg user'],
  ...
  static normalizeRole(role) {
    ...
    if (roleLower.includes('manager') || roleLower.includes('менеджер') || roleLower.includes('руководитель')) {
      return 'manager';
    }
  ...
  static async validateImportFile(filePath) {
    ...
    try {
      ...
      requiredFields.forEach(field => {
        const hasField = availableFields.some(available => 
      ...
      userData.forEach((row, index) => {
        ...
        if (username && usernames.includes(username)) {
          warnings.push(`Строка ${index + 1}: дублирующийся логин "${username}"`);
        } else if (username) {
