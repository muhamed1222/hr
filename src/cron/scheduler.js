...
const { cron: cronLogger, error } = require('../utils/logger');
...
class ReminderScheduler {
  ...
  setupSchedules() {
    // 09:50 - Утренние напоминания (не отметили приход)
    ...
    // 17:50 - Вечерние напоминания (не сдали отчёт)
    ...
    if (process.env.NODE_ENV === 'development' && process.env.CRON_TEST === 'true') {
      cron.schedule('*/1 * * * *', async () => {
        cronLogger('Тестовый запуск cron', { time: moment().format('HH:mm:ss') });
  ...
  async sendMorningReminders() {
    try {
      ...
      for (const user of users) {
        ...
        if (result) {
          sent++;
          cronLogger('Утреннее напоминание отправлено', { userName: user.name });
        }
      ...
      cronLogger('Утренние напоминания завершены', { sent, total: users.length });
    } catch (err) {
  ...
  async sendLunchStartReminders() {
    try {
      ...
      for (const user of users) {
        ...
        if (result) {
          sent++;
          cronLogger('Напоминание об обеде отправлено', { userName: user.name });
        }
        
        await this.sleep(REMINDER_DELAY_MS);
      }

      cronLogger('Напоминания об обеде завершены', { sent, total: users.length });
    } catch (err) {
  ...
  async sendLunchEndReminders() {
    try {
      ...
      if (users.length === 0) {
        cronLogger('Все вернулись с обеда');
        return;
      }
      ...
      for (const user of users) {
        ...
        if (result) {
          sent++;
          cronLogger('Напоминание об окончании обеда отправлено', { userName: user.name });
        }
        
        await this.sleep(REMINDER_DELAY_MS);
      }

      cronLogger('Напоминания об окончании обеда завершены', { sent, total: users.length });
    } catch (err) {
  ...
  async sendEveningReminders() {
    try {
      ...
      for (const user of users) {
        ...
        if (result) {
          sent++;
          cronLogger('Вечернее напоминание отправлено', { userName: user.name });
        }
      ...
      cronLogger('Вечерние напоминания завершены', { sent, total: users.length });
    } catch (err) {
    ...
    try {
      ...
      if (global.emitEvent) {
        const statsData = {
          ...
          absentEmployees: (stats.totalEmployees || 0) - (stats.presentToday || 0),
      ...
      for (const manager of managers) {
        ...
        if (result) {
          sent++;
          cronLogger('Статистика отправлена менеджеру', { managerName: manager.name });
        }
        
        await this.sleep(REMINDER_DELAY_MS);
      }

      cronLogger('Статистика отправлена менеджерам', { sent, total: managers.length });
    } catch (err) {
  ...
  async sendTestReminders() {
    ...
    if (testUser) {
      ...
      cronLogger('Тестовое напоминание отправлено', { userName: testUser.name });
    }
  }
  ...
  getNextRuns() {
    ...
    return {
      morning: today.clone().hour(9).minute(50).second(0),
      lunchStart: today.clone().hour(14).minute(0).second(0),
      lunchEnd: today.clone().hour(15).minute(0).second(0),
      evening: today.clone().hour(17).minute(50).second(0),
      stats: today.clone().hour(18).minute(30).second(0)
    };
  }
}
