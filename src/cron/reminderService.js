...
async function getUsersForMorningReminder() {
  try {
    ...
    // // info(`🌅 Утренние напоминания: найдено ${usersToRemind.length} пользователей`);
    return usersToRemind;
  } catch (error) {
    error("❌ Ошибка получения пользователей для утренних напоминаний:", error);
    return [];
  }
}
...
async function getUsersForLunchStartReminder() {
  try {
    ...
    // // info(`🍱 Напоминания об обеде: найдено ${users.length} пользователей`);
    return users;
  } catch (error) {
    error("❌ Ошибка получения пользователей для напоминаний об обеде:", error);
    return [];
  }
}
...
async function getUsersForLunchEndReminder() {
  try {
    ...
    // // info(`🔙 Напоминания об окончании обеда: найдено ${users.length} пользователей`);
    return users;
  } catch (error) {
    error(
      "❌ Ошибка получения пользователей для напоминаний об окончании обеда:",
      error,
    );
    return [];
  }
...
async function getUsersForEveningReminder() {
  try {
    ...
    // // info(`🌆 Вечерние напоминания: найдено ${users.length} пользователей`);
    return users;
  } catch (error) {
    error("❌ Ошибка получения пользователей для вечерних напоминаний:", error);
    return [];
  }
}
...
async function getReminderStats() {
  try {
    ...
    error("❌ Ошибка получения статистики напоминаний:", error);
