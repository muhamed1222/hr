...
router.get('/', async (req, res) => {
  try {
    ...
    const { rows: users, count } = await User.findAndCountAll({
      where: whereClause,
      attributes: ['id', 'telegramId', 'name', 'username', 'role', 'status', 'createdAt'],
      order: [['name', 'ASC']],
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const user = await User.findByPk(id, {
      attributes: ['id', 'telegramId', 'name', 'username', 'role', 'status', 'createdAt', 'updatedAt']
    });
...
router.get('/:id/stats', async (req, res) => {
  try {
    ...
    const stats = {
      ...
      totalWorkMinutes: workLogs.reduce((sum, log) => sum + (log.totalMinutes || 0), 0),
      averageWorkHours: 0,
      workModeStats: {
      ...
      daysWithoutReport: workLogs.filter(log => !log.dailyReport || log.dailyReport.trim() === '').length
    };
...
router.get('/ranking/reliability', async (req, res) => {
  try {
    ...
    const userRankings = await Promise.all(
      users.map(async (user) => {
        ...
        const stats = {
          ...
          daysWithoutReport: workLogs.filter(log => !log.dailyReport || log.dailyReport.trim() === '').length,
          averageWorkMinutes: workLogs.length > 0 
            ? workLogs.reduce((sum, log) => sum + (log.totalMinutes || 0), 0) / workLogs.length 
            : 0
        };
    ...
    res.json({
      success: true,
      data: userRankings.slice(0, parseInt(limit))
    });
  } catch (error) {
...
function calculateReliabilityScore(stats) {
  ...
  score -= Math.min(30, lateRate * 100);

  // Штраф за отсутствие отчётов (максимум -25 баллов)
  const noReportRate = stats.daysWithoutReport / stats.totalDays;
  score -= Math.min(25, noReportRate * 100);

  // Бонус за среднее время работы больше 8 часов (максимум +15 баллов)
  if (stats.averageWorkMinutes > 480) { // 8 часов = 480 минут
    const overtimeBonus = Math.min(15, (stats.averageWorkMinutes - 480) / 60 * 5);
    score += overtimeBonus;
  }

  // Штраф за среднее время работы меньше 7 часов (максимум -20 баллов)
  if (stats.averageWorkMinutes < 420) { // 7 часов = 420 минут
    const undertimePenalty = Math.min(20, (420 - stats.averageWorkMinutes) / 60 * 10);
