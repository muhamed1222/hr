...
async function notifyUserPromoted(userData) {
  const { telegramId, firstName, lastName, oldRole, newRole, promotedBy } = userData;
...
${!canUseUrlInTelegram(WEB_APP_URL) ? '\n🔗 Веб-панель будет доступна после настройки публичного URL.' : ''}
  ...
  if (canUseUrlInTelegram(WEB_APP_URL)) {
    options.reply_markup = {
      inline_keyboard: [
        ...
        [
          {
            text: newRole === 'admin' ? '👑 Управление системой' : '👥 Моя команда',
            url: newRole === 'admin'
  ...
  try {
    ...
    if (result) {
      info(`🎉 Уведомление о повышении отправлено: ${firstName} ${oldRole} → ${newRole}`);
    }

    return result;
  } catch (err) {
    error('❌ Ошибка отправки уведомления о повышении:', err);
    throw err;
  }
}
