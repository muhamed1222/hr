...
${!canUseUrlInTelegram(WEB_APP_URL) ? '\n🔗 Веб-интерфейс будет доступен после настройки публичного URL.' : ''}
  ...
  if (canUseUrlInTelegram(WEB_APP_URL)) {
    options.reply_markup = {
      inline_keyboard: [
        ...
        [
          {
            text: role === 'manager' || role === 'admin' ? '👥 Управление командой' : '📊 Моя статистика',
            url: role === 'manager' || role === 'admin' 
  ...
  try {
    ...
    if (result) {
      info(`✅ Приветственное сообщение отправлено пользователю ${firstName} (${telegramId})`);
    }

    return result;
  } catch (err) {
    error('❌ Ошибка отправки приветственного сообщения:', err);
    throw err;
  }
}
